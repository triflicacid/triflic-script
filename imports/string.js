...
module.exports = rs => {
    ...
    rs.define(new RunspaceBuiltinFunction(rs, 'join', { arr: 'array', seperator: '?string' }, ({ arr, seperator }) => new StringValue(rs, arr.toPrimitive('array').map(v => v.toString()).join(seperator ?? '')), 'String: Join elements in an array by <seperator> to form a string'));
