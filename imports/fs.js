...
function main(rs) {
  ...
  rs.defineFunc(new RunspaceBuiltinFunction(rs, 'fexists', { filepath: 'string' }, async ({ filepath }) => new BoolValue(rs, await fexists(filepath.toString())), 'does the given file exist?'));
  ...
  }, 'write to a file (creates is not exist)'));
